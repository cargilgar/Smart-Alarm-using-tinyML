...
void loop(){
    ...
    while (millis() < (startTime + readingTime)){               // Reading pulse sensor for readingTime seconds
        ...
        if ((sensorValue >= threshold) && (!counted) && ((newBeatTime - lastTime) > 420)){  // Check threshold, wave rising and IBI > 420 ms

            // Serial.println(sensorValue);                  // Print Sensor value (just for checking) 
            ...
            heartRate = heartRate + hr;                      // Add BPMs calculated
            hr = 0;                                          // Reset the value to use it again in the next beat
           
            
        } else if (sensorValue < threshold){                 // Check the wave is decreasing below threshold value
